...
# ref https://pythonprogramming.net/getting-data-sp-500-index-value-comparison
#           /?completed=/using-pandas-structure-process-data/
# 1. data
# 2. labeling
...
def Key_Stats(gather=["Total Debt/Equity",
    ...
    statspath = path+'/_KeyStats'   # stats directory
    ...
    sp500_df = pd.DataFrame.from_csv(homepath+"/YAHOO-INDEX_GSPC.csv")

    ticker_list = []

    for each_dir in stock_list[1:]:  # 25 is test element number
        each_file = os.listdir(each_dir)

        dir_split = each_dir.split("/")
        ticker = dir_split[len(dir_split)-1]
            ...
            for file in each_file:
                ...
                try:
                    ...
                    try:
                        ...
                        sp500_date = sp500_date.strftime('%Y-%m-%d')
                        row = sp500_df[(sp500_df.index == sp500_date)]
                        sp500_value = float(row["Adjusted Close"])
                    except:
                        sp500_time = unix_time
                        lenRow = 0
                        while lenRow == 0:
                            sp500_time = sp500_time - 86400
                            sp500_date = datetime.fromtimestamp(sp500_time)
                            sp500_date = sp500_date.strftime('%Y-%m-%d')
                            row = sp500_df[(sp500_df.index == sp500_date)]
                            lenRow = len(row)
                        sp500_value = float(row["Adjusted Close"])
                    ...
                    if value_list.count("N/A") > 0:
                        ...
                        df2 = df2.reindex(columns=dfSequence)
